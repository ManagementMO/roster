...
`@npmmo/roster@0.0.4` is the current public npm release, with the `roster` executable and unchanged supported Node baseline. The owner separately reviewed PR #46 and approved its exact artifact; publication completed on 2026-09-14 after merge CI/CodeQL and npm browser confirmation. The unauthenticated public download matched the approved SHA-256. The publication, verification scope, and owner attestation are recorded in [release readiness](docs/release-readiness.md) and [PROVENANCE](docs/PROVENANCE.md).
...
The public `0.0.3` repair-only stale-resolution finding is fixed by `0.0.4`. Its pre-release verification passed 572 tests, all 19 configurations with required manifest repair, six upgrades, six offline reruns, Windows network traces, and four repeated Codex scenarios. The public matrix then passed with 157 PASS, no failures or blocked cases, and eight explicitly unrun platform-specific network cases; all five copied-command legs passed. [Release readiness](docs/release-readiness.md) retains the exact scope and the original `0.0.3` failure. Claude model calls remain blocked by account credit; Cursor/OpenClaw GUI flows, ARMv7 dense support, and the recorded upstream unusual-path limits are not claimed resolved.
