...
// @roster/cli tarball. Everything (install prefix, storage, htpasswd, token,
...
if (cmd === "start") {
  ...
  if (!res.ok || !body.token) throw new Error(`could not create staging user: ${res.status} ${JSON.stringify(body)}`);
  const publisherRc = path.join(conf, "publisher.npmrc");
  fs.writeFileSync(publisherRc, `//127.0.0.1:${port}/:_authToken=${body.token}\nregistry=${url}\n`, { mode: 0o600 });

  run(npmCmd, ["publish", tarball, "--registry", url, "--userconfig", publisherRc, "--ignore-scripts"], { cwd: dir, env: { ...toolEnv, npm_config_cache: path.join(dir, "publish-cache") } });

  const packument = await (await fetch(`${url}@roster%2fcli`)).json();
