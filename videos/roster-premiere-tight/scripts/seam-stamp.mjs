...
//   exit.travel / scale and entry.scale — restrained travel for this short revision
// Explicit fromTo exit states keep arbitrary seeks from reusing stale entry transforms.
...
for (const seam of ledger.seams) {
  ...
  if (seam.entry?.axis === "z") {
    ...
    zEntries.set(
      seam.entry.selector,
      seam.entry.dir === -1
        ? { scale: seam.entry.scale ?? 1.25, blur } // pull: arrives oversized
        : { scale: seam.entry.scale ?? 0.78, blur },
...
for (const seam of ledger.seams) {
  ...
  if (ex.axis === "z") {
    ...
    const exDur = ex.dur ?? 0.21,
      enDur = en.dur ?? 0.5;
    const exScale = ex.scale ?? (ex.dir === -1 ? 0.8 : 1.18);
    const enFrom = en.scale ?? (ex.dir === -1 ? 1.25 : 0.78);
    emit(
      `tl.fromTo("${ex.selector}", { scale: 1, filter: "blur(0px)" }, { scale: ${exScale}, filter: "blur(${blur}px)", duration: ${exDur}, ease: "power3.in", immediateRender: false }, ${round(cut - exDur)});`,
    );
    emit(
      `tl.fromTo("${ex.selector}", { autoAlpha: 1 }, { autoAlpha: 0, duration: ${exDur}, ease: "none", immediateRender: false }, ${round(cut - exDur)});`,
    );
    ...
    emit(
      `tl.fromTo("${ex.selector}", { ${prop}: 0, autoAlpha: 1 }, { ${prop}: ${(ex.travel ?? 12) * ex.dir}, autoAlpha: 0, duration: ${exDur}, ease: "power3.in", immediateRender: false }, ${round(cut - exDur)});`,
    );
    emit(`tl.set("${ex.selector}", { autoAlpha: 0 }, ${cut});`);
    emit(
