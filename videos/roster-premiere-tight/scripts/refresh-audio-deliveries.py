...
@contextmanager
def restore_deliveries_on_error(paths):
    """Restore the complete input set if a later file or audit fails."""
    with TemporaryDirectory(prefix="roster-audio-refresh-") as temporary_dir:
        originals = {}
        for index, path in enumerate(dict.fromkeys(paths)):
            backup = Path(temporary_dir) / str(index)
            if path.exists():
...
# Snapshot every destination before changing either edition. A late remux,
# probe, or audit error restores exports, feed audio/metadata and all sidecars.
destinations = []
for folder, prefix in [(ROOT, "roster-tight"), (ROOT.parent / "roster-premiere-feed", "roster-feed")]:
    for suffix in ["120fps", "60fps", "preview"]:
        filename = f"{prefix}-{suffix}.mp4"
        destinations.extend([folder / "renders" / filename,
                             folder / "verification/before-pocket-groove/renders" / filename])
    for name in [meta["bgm"]["path"], "assets/audio/five-in-motion.m4a",
                 "assets/audio/roster-pocket-groove.m4a", "audio_meta.json",
                 "verification/pocket-groove-delivery.json", "verification/delivery.json",
                 "verification/media-probe.json"]:
        destinations.append(folder / name)

with restore_deliveries_on_error(destinations):
    for folder, prefix in [(ROOT, "roster-tight"), (ROOT.parent / "roster-premiere-feed", "roster-feed")]:
        if folder != ROOT:
            for name in [meta["bgm"]["path"], "assets/audio/five-in-motion.m4a",
                         "assets/audio/roster-pocket-groove.m4a", "audio_meta.json"]:
        ...
        if delivery_path.exists():
            ...
            for item in delivery["files"]:
                delivered_file = folder / item["path"]
                item["sha256"] = hashlib.sha256(delivered_file.read_bytes()).hexdigest()
                item["bytes"] = delivered_file.stat().st_size
                if delivered_file.suffix in [".wav", ".m4a"]:
                    audio_probe = probe_media(delivered_file)
                    audio_probe["format"]["filename"] = item["path"]
                    probes[item["path"]] = audio_probe
            delivery["loudnessSummary"] = (
                f"Encoded AAC: {loudness['input_i']} LUFS integrated; "
                f"{loudness['input_lra']} LU loudness range; "
                f"{loudness['input_tp']} dBTP true peak."
            )
            delivery["detectedSilence"] = re.findall(r"silence_(?:start|end):[^\n]+", audit)
