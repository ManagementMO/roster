...
The client normally starts the launcher written by sync. A source install uses Node and an absolute path to that installation. A verified global installation uses `roster serve`. An npm-cache invocation, after publication, writes the scoped `npx -y @roster/cli serve` form so a pruned cache can be resolved again.
