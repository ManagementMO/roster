...
`@npmmo/roster@0.0.3` is the current public npm release, with the `roster` executable and unchanged supported Node baseline. The owner reviewed PR #45 and approved the exact artifact; publication completed on 2026-09-14 after merge CI/CodeQL and npm browser confirmation. Public download matched the approved SHA-256. The publication, verification scope, and owner attestation are recorded in [release readiness](docs/release-readiness.md) and [PROVENANCE](docs/PROVENANCE.md).
...
The `0.0.3` candidate passed 568 tests, 19 portability configurations, six network-removed Alpine reruns, selected upgrades, Windows network traces, and four signed-in Codex scenarios before publication. The broader public checks passed all five copied-command and upgrade legs but exposed a damaged-runtime repair false error on Linux and Windows Node 24.20.0. The owner approved preparation of `0.0.4`: resolve the repaired package inside the fresh post-install probe and test repair before release. That follow-up is not yet public or approved for publication. Claude model calls remain blocked by account credit; Cursor/OpenClaw GUI flows, ARMv7 dense support, and the recorded upstream unusual-path limits are not claimed resolved. See [release readiness](docs/release-readiness.md) for exact hashes, failures, and scope.
...
| Gate | Status |
|---|---|
| Unit/integration tests | **409 passing across 16 files** after independent meta-verification (369/14 on the `9741ff6` base it audited; +40 tests covering permissions, platform-neutral shutdown, receipt truthfulness, persistent-slot lock recovery, and publish/install behavior) |
| CI/CD workflow | The release-readiness head and its status-only follow-ups passed lint · build-test (Ubuntu/macOS/Windows on Node 24 plus Ubuntu on the exact 22.13.x floor) · real-server E2E + fail-probes · Combine · clean external install on Ubuntu + Windows · live MiniLM · audit/secret scan · CodeQL. Semgrep and Sourcery also passed, but they are **GitHub Apps configured outside this repo and they run on pull requests only** — they are not in `.github/workflows`, do not run on pushes to `main`, and Sourcery reports `skipped` on some runs, so they are reported here as supporting evidence, never as a repo-reproducible gate. |
...
2b. **Branch protection** ✅ **Active ruleset.** GitHub's `main-protection` ruleset is active on `main` and requires the documented CI/CodeQL status checks while blocking deletion and non-fast-forward updates. The classic branch-protection endpoint remains unconfigured, and the ruleset currently requires zero approving reviews; review enforcement can be strengthened later if you want it.
...
**F. Launch mechanics (day-of checklist, small but must not be forgotten):** launch assets (GIFs, exposé, posts) · publish the already-bundled `@roster/cli` package (no code flip required: global installs write `roster serve`, npx-cache installs write self-healing `npx -y @roster/cli serve`) · keep README install lines aligned with the registry state · complete the owner's legal/brand gate.
...
- **CI/CD:** `main` retains the full Ubuntu/macOS/Windows matrix, exact Node 22.13.x floor, Router E2E, Combine, live MiniLM, dependency/secret scan, CodeQL, and Semgrep gates (see §1).
...
- **Round 6 — independent clean-room review (2026-08-17):** a fresh reviewer re-ran every gate and probe from scratch on `9741ff6`, wrote 160 new adversarial checks (CLI trust path 44/44, classifier/ReDoS/JSONC/identity 116/116, transparent fidelity: zero field differences across 14 tools × 6 fields), and **mutation-audited the test suite itself: 15 load-bearing fixes reverted one at a time, 15/15 mutants killed** — no vacuous lock among them. It found the packaging launch blocker (R6-03), a permissions gap (R6-01), missing branch protection (R6-02), four doc/cleanup items, and — by chasing a 1-in-8 test failure instead of re-running it — **a permanently wedged config lock (R6-08)**: an ownerless lock directory could never be reclaimed, so `roster sync` would fail after a 5s wait forever until `~/.roster/locks` was deleted by hand. Full record in `docs/lab/review-round6-hardening.md`.
- **Round 7 lock closure (2026-08-21):** the remaining rename-claim gap reproduced as one release failure in 100 focused four-process runs. The canonical lock is now a persistent slot, `owner.json` is the atomic mutex, and a single fixed in-directory `.reclaim` marker serializes dead-owner replacement without ever moving the canonical path. Three old-code design-invariant tests failed before the rewrite; claim crash/dead/live recovery is covered; the same campaign passed **200/200** after it.
- **Independent meta-verification (2026-08-21):** re-ran the merged artifact from scratch and found the one remaining platform asymmetry: directory cleanup retried transient Windows handle errors, but `owner.json` create/remove did not. Owner-file creation now treats those errors as bounded lock contention, removal retries them for at most 500ms, and non-transient faults still surface; clean external install now runs on Windows in CI as well as Ubuntu.
- **Post–Round 5 closure (2026-07-30, branch `fix/post-round5-closure`, PR #11 — not PR #10):** worked the audit backlog left after PR #10 merged, then independently re-reviewed its own fixes. The follow-up closed permissive invalid-schema isolation, hash-upgrade tombstone drift, intermediate-directory verifier races, overbroad staging cleanup, and a SKILL.md symlink-warning race. Each new lock was reproduced, fixed under TDD, and mutation-checked with byte-identical restoration. Its historical gate was **366/366**; later hardening and packaging commits brought the current gate to **409/409**, with the hosted matrix green, including the dependency audit. Full claims matrix in `docs/lab/post-round5-closure.md`.
