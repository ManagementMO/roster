...
> **Release update (2026-09-13):** `@npmmo/roster@0.0.2` is the current public release under the `npmmo` account, with executable `roster`. The owner confirmed the required human review for `0.0.1` and separately authorized the documentation-only `0.0.2` patch; the runtime bundles are unchanged. Fresh public npm/npx installs and `0.0.1` upgrades passed on Linux, Windows, and macOS within the recorded scope. Older `@roster/cli` references below are decision history, not the package to install. League signing, named scores, domains, and service deployment remain separate owner gates. Current evidence: `docs/release-readiness.md` and `docs/PROVENANCE.md`.

> **Deployment and verification follow-up (2026-09-14):** the owner approved the npx-first frontend and production deployment to Vercel project `roster`. The preferred public website is `https://roster-router.vercel.app`, attached to the project and updated by the native GitHub integration from `main`; a real branch preview and automatic post-merge production build were verified. Genuine Windows non-admin x64/ARM64 checks are now complete. Optional native dense-runtime failures on Alpine/musl and Intel macOS led to owner-approved work on a `0.0.3` WASM-fallback/readiness candidate. Its 19-configuration artifact matrix, Windows process-attributed network traces, and four real Codex CLI flows have passed; the candidate still awaits release approval and is not yet public npm. League signing, named scores, telemetry uploads, and new paid services remain outside this work. Details: `docs/release-readiness.md`.
