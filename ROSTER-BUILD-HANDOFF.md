...
| Embedding model | EmbeddingGemma-300M (ONNX confirmed in transformers.js/Node; first-run HF fetch — avoids Gemma-Terms flow-down compliance + npm bloat; check for an Apache-2.0 Gemma-4-gen successor at build time); `--quality` = Qwen3-Embedding-0.6B (Apache 2.0, base of ToolRet-SOTA retrievers); low-RAM fallback all-MiniLM-L6-v2 |
