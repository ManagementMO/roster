...
async function openPage(compUrl) {
  ...
  // Preview's runtime defaults to 30fps even when the composition declares120.
  // Use the renderer's own documented-in-runtime seek configuration, before bootstrap,
  // so cut+1/fps measures a real target frame rather than snapping back before the cut.
  await cdp.send("Page.addScriptToEvaluateOnNewDocument", {
    source: `window.__HF_EXPORT_RENDER_SEEK_CONFIG = { fps: ${FPS}, fpsSource: "render-options" };`,
  }, sessionId);
  await cdp.send(
    "Emulation.setDeviceMetricsOverride",
    { width: 1080, height: 1350, deviceScaleFactor: 1, mobile: false },
    sessionId,
  );
...
const HARNESS = `window.__seamGate = {
  ...
  read(sel){
    ...
    const onscreen = r.right > 0 && r.bottom > 0 && r.left < 1080 && r.top < 1350;
...
async function verify() {
  ...
  for (const seam of ledger.seams) {
    ...
    // A cross-scene asset must have one owner on every frame, including both
    // source/carrier and carrier/destination handoffs. Also seek backwards to
    // catch state that only works after sequential playback.
    const handoff = seam.carrier;
    if (handoff?.source && handoff?.destination) {
      const selectors = [...new Set([handoff.source, handoff.out, handoff.in, handoff.destination])];
      const firstFrame = Math.round(handoff.start * fps) - 2;
      const lastFrame = Math.round(handoff.end * fps) + 2;
      const times = Array.from({ length: lastFrame - firstFrame + 1 }, (_, i) => (firstFrame + i) / fps);
      const sampleAsset = async (time) => evalJs(`(() => {
        const sample = __seamGate.sample(${time}, ${JSON.stringify(selectors)});
        for (const [selector, box] of Object.entries(sample)) {
          if (!box) continue;
          const el = document.querySelector(selector);
          const style = getComputedStyle(el);
          const sx = box.w / el.offsetWidth, sy = box.h / el.offsetHeight;
          const l = parseFloat(style.paddingLeft) * sx, r = parseFloat(style.paddingRight) * sx;
          const t = parseFloat(style.paddingTop) * sy, b = parseFloat(style.paddingBottom) * sy;
          box.cx += (l-r)/2; box.cy += (t-b)/2;
          box.w -= l+r; box.h -= t+b;
        }
        return sample;
      })()`);
      ...
      for (const time of times) {
        const sample = await sampleAsset(time);
        forward.set(time, sample);
        const owners = Object.entries(sample).filter(([, box]) => visible(box));
        if (owners.length !== 1 || owners[0][1].op < 0.99) ownershipFailures.push(time);
      }
      add("handoff-ownership", ownershipFailures.length ? "FAIL" : "PASS",
        ownershipFailures.length ? `missing, faded or duplicate asset at ${ownershipFailures.slice(0, 5).join(", ")}s` : `one opaque asset on all ${times.length} sampled frames at ${fps}fps`);

      let maxSeekDelta = 0;
      for (const time of [...times].reverse()) {
        const reverse = await sampleAsset(time);
        for (const selector of selectors) {
          const a = forward.get(time)[selector], b = reverse[selector];
          if (!a || !b) { maxSeekDelta = Infinity; continue; }
          for (const key of ["cx", "cy", "w", "h", "op"]) maxSeekDelta = Math.max(maxSeekDelta, Math.abs(a[key] - b[key]));
        }
      }
      add("handoff-seeking", maxSeekDelta < 0.05 ? "PASS" : "FAIL", `forward/reverse maximum difference ${maxSeekDelta.toFixed(4)}`);

      for (const [label, boundary, outgoing, incoming] of [
        ["source", handoff.start, handoff.source, handoff.out],
        ["destination", handoff.end, handoff.in, handoff.destination],
      ]) {
        const before = (await sampleAsset(boundary - dt))[outgoing];
        const after = (await sampleAsset(boundary))[incoming];
        const delta = !before || !after ? Infinity : Math.max(...["cx", "cy", "w", "h"].map(key => Math.abs(before[key] - after[key])));
        add(`handoff-${label}`, delta < 1 ? "PASS" : "FAIL", `painted asset geometry changes ${delta.toFixed(3)}px at ownership transfer`);
      }
    }
