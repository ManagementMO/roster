...
function drawTerminalBeat(ctx, time) {
  ...
  // Release each object from the terminal on the same smooth Bézier it uses
  // as its connector. The path is drawn progressively, so nothing is already
  // sitting at the destination when the scene begins.
  ...
  routes.forEach(({ tool, start, control1, control2, end }, index) => {
    ...
    if (local > 0.01) {
      ...
      drawToolObject(ctx, tool, point[0], point[1], 238, 128, cardScale, 8 * (1 - local), local, clamp(local * 1.8), true, false, 0.04);
    }
  });
  ctx.restore();
...
function drawFocusBeat(ctx, time) {
  ...
  // Let the shortlist settle before the focus carriage begins moving. The old
  // scan started while cards were still arriving, which made the whole row
  // feel like one noisy, AI-generated motion system.
  const scan = ease.inOut(span(localTime, 0.72, 2.48));
  const lensX = mix(294, 1660, scan);
  const railStart = [250, 790];
  const railEnd = [1670, 790];
  ctx.save();
  ctx.globalAlpha = boardIn;
  ctx.strokeStyle = "rgba(8,10,10,0.92)";
  ctx.lineWidth = 24;
  ctx.lineCap = "round";
  ctx.beginPath();
  ...
  ctx.lineTo(...railEnd);
  ctx.stroke();
  ctx.strokeStyle = rgba(COLOR.glassDim, 0.62);
  ctx.lineWidth = 7;
  ctx.beginPath();
  ctx.moveTo(...railStart);
  ctx.lineTo(mix(railStart[0], railEnd[0], scan), railStart[1]);
  ctx.stroke();
  const railX = mix(railStart[0], railEnd[0], scan);
  fillRoundRect(ctx, railX - 28, railStart[1] - 7, 56, 14, 7, rgba(COLOR.glassLight, 0.9));
  fillRoundRect(ctx, railX - 18, railStart[1] - 3, 36, 6, 3, rgba(COLOR.white, 0.9));
  ctx.restore();
  ...
  positions.forEach(([x, y, rotation], index) => {
    const cardStart = 0.14 + index * 0.13;
    const cardIn = ease.out5(span(localTime, cardStart, cardStart + 0.5));
    const proximity = ease.inOut(clamp(1 - Math.abs(lensX - x) / 320));
    const tool = STARTERS[index];
    const cardY = y - proximity * 14;
    const cardScale = mix(0.86, 1, cardIn) + proximity * 0.04;
    const cardAlpha = mix(0.52, 1, Math.max(proximity, cardIn * 0.54));
    ...
    drawToolObject(ctx, tool, x, cardY, 282, 158, cardScale, cardRotation, cardIn, clamp(cardIn * 1.8), true, false, 0);
    ctx.restore();
    if (proximity > 0.18) {
      ctx.save();
      ...
      ctx.globalAlpha = boardIn * ease.inOut(clamp((proximity - 0.1) / 0.9)) * 0.84;
...
function drawIdentityBeat(ctx, time) {
  ...
  // Keep the orbit dominant while giving the new white mark a little more
  // breathing room in the final lockup.
  drawRosterLogo(ctx, stage.x, stage.y, 236 * mark, COLOR.roster, mark, mark, true, true, 1.15);
