...
> **Release amendment (2026-09-13):** the published CLI preview is `@npmmo/roster@0.0.2` (a README-link-only patch over the reviewed `0.0.1` runtime), with executable `roster`, and the approved supported-runtime range is Node `^22.17.0 || >=24.2.0`. Affected Windows/libuv versions are refused before local state changes, never accommodated by weakened file-identity checks. POSIX shutdown owns an isolated backend process group and must terminate its ordinary non-detached descendants without signalling unrelated processes. Windows npm and saved npx launchers must execute through a platform-correct path. Release evidence and the owner-confirmed human review are recorded in `docs/release-readiness.md` and `docs/PROVENANCE.md`; these requirements remain in force for later changes and do not waive the laws below.
...
**Post-install repair correction (2026-09-14, owner-authorized `0.0.4` preparation):** public `0.0.3` verification reproduced a repair-only false failure after an earlier package lookup cached missing metadata. npm restored the runtime, and a fresh CLI process reported READY, but the installing process reused stale resolution state. Resolve and validate the owned package inside the fresh bounded post-install probe, preserving ownership checks and lexical fallback. Regression coverage must include a repaired missing manifest and an entrypoint changed by installation; the packaged pre-release matrix must exercise repair before publication. The published `0.0.3` archive remains immutable, and `0.0.4` needs its own artifact approval.
...
- **Sixth Man — SUGGEST-ONLY at launch (owner decision 2026-07-04, red-team accepted):** on failure class ∈ {timeout, transport, http_5xx, tool_fail:internal}, return a structured error carrying `{"_roster":{"suggested_alternate":{"tool":Y,"reason":Z,"args_compatible":bool}}}` — the AGENT decides and re-calls; Roster never auto-fires a second tool. `args_compatible` is computed by validating the original args against the candidate's `inputSchema` (equivalent tools rarely share schemas — the flag tells the agent whether its args carry over or need rebuilding). Candidate selection (M0 implementation, 2026-07-04): next candidate from the same draft's ranking with a DIFFERENT source than the failed tool (cosine-threshold refinement lands with the dense rung). Suggestion logging IS implemented: every suggestion recorded in the `suggestion` table; `taken` flips when the agent calls the suggested capability in-session — that field data qualifies auto-substitution post-launch (read-only/idempotent classes first, never writes).
...
- **Base model [resolved 2026-07-03]:** default **EmbeddingGemma-300M** — ONNX + transformers.js/Node confirmed working (`onnx-community/embeddinggemma-300m-ONNX`; in-browser/Node demos exist). **Weights are fetched at first run, not bundled — corrected rationale (verified 2026-07-04): the Gemma Terms of Use DO permit redistribution, but only with flow-down conditions (use-restrictions as an enforceable provision + agreement copy + NOTICE file per [ai.google.dev/gemma/terms](https://ai.google.dev/gemma/terms)); first-run fetch avoids that compliance overhead entirely AND keeps the npm package small (shipping ~200MB in node_modules is hostile regardless of license). Build-time check: the Gemma 4 generation moved to Apache 2.0 — if an Apache-licensed successor embedding model exists by build time, prefer it and the friction disappears.** Implementation note: transformers.js does not support simultaneous WASM/WebGPU session execution — serialize all embedding calls through a single queue. **Runtime rules (final, owner-delegated 2026-07-04):** auto-select the model by machine — EmbeddingGemma-300M when ≥8GB RAM, all-MiniLM-L6-v2 below (both auto-fetched in the background; FTS5 serves meanwhile either way); **lazy-load** the embedder on first embed and unload after ~10 min idle, so resident RAM is near zero for light users and the RT2 N-clients×model concern shrinks until the shared daemon lands; store vectors **Matryoshka-truncated to 256 dims** (EmbeddingGemma supports this natively — ~3× smaller index, faster search, negligible quality loss at ≤2K-entry scale). `--quality` tier: **Qwen3-Embedding-0.6B (Apache 2.0)** — notable because the ToolRet-SOTA retrievers (Tool-Embed) are built on exactly this base; if Tool-Embed's own weights are ever released open, they slot in here. Low-RAM fallback: all-MiniLM-L6-v2 (46MB). OATS refines whichever base is active.
...
timeout_ms: 30000
```
**Implementation note (2026-07-04, reality-wins amendments):** the shipped task format uses `timeoutMs` and DECLARATIVE verifiers (`fileEquals`/`fileContains`/`fileExists`/`fileAbsent`/`resultContains`) executed by the runner — safer than arbitrary verify scripts and equally MCPMark-faithful (end-state checks); script-based verifiers can be added later behind review. Setup file names AND contents are templated; sandbox paths are realpath-resolved (symlinked tmpdirs caused false failures) and containment-checked (contributed suites are untrusted).
...
**Verifier-certification protocol (NR1 — this is how "automated testing with agents" stays publishable):** (1) an agent DRAFTS each task + verify script; (2) a second adversarial agent attacks it (wrong-field checks, false-pass/false-fail hunting); (3) mutation testing runs the verifier against seeded known-bad sandbox states — it must catch them all; (4) a HUMAN certifies (~10–20 min: run the pass case, force a fail case, confirm it matches the server's real semantics) and the task gets `signed: true` provenance; (5) **only signed tasks feed named public scores** — unsigned tasks may run for internal/anonymized stats only. Same pipeline for skills tasks. After signing, everything is automated forever: weekly rerun CI, new servers against existing signed suites, drift alerts, box scores — the human cost is a one-time certification gate, not ongoing labor.
...
|---|---|---|
| Claude Code | **`~/.claude.json`** (user scope — verified on a live machine 2026-07-04; `~/.claude/settings.json` does NOT hold mcpServers) + project `.mcp.json`; or `claude mcp add` | JSON `mcpServers` |
...
Latency: `draft` <50ms local (embedding search over ≤2,000 tools); passthrough overhead <5ms. Zero network required after first run — and even the first run serves instantly in lexical mode (dense retrieval upgrades silently when the one-time, checksummed model download completes in the background; weights are never bundled — fetch-at-first-run avoids Gemma-Terms flow-down compliance and npm bloat; permanently-offline machines simply stay in lexical mode, stated honestly in docs). Cross-platform: macOS + Linux first, Windows before launch (config paths differ — test). Licenses MIT. **Integrity law: no fabricated numbers anywhere, ever** — receipts labeled as estimates, League shows CIs and n, every public stat traceable to a run artifact. Secrets: env passthrough to backends only; never persisted by us, never logged.
...
- **M0 (weekend):** `roster init && roster serve` on a machine with Claude Code: transparent mode proxies 3 real servers (filesystem, fetch/search, github) end-to-end; outcomes in SQLite; receipt prints; unit tests green. *Verify: run a real Claude Code session through it; diff behavior vs direct config (must be identical).*
  **M0 status (2026-07-04, built + adversarially reviewed + fixed):** DONE with real-server e2e evidence (docs/verification/): parity vs direct connection, outcomes+privacy verified, five-mode draft/call live, Combine 8/8 deterministic vs the real filesystem server. Deliberate M0 deferrals (spec unchanged, land M1–M2): roster-cache TTL/`ttlMs`, ~10K-token adaptive engagement rule, backend health-checks/lazy-connect/concurrency caps, per-serve schema-token measurement on the receipt, OATS negatives currently = all attributable failures (conservative superset of "drafted #1 but failed" — revisit with rank logging).
  **Deep-sweep amendments (2026-07-04, reality-wins):** eject restores the OLDEST pristine backup of the current era, and the era closes on restore (deliberate improvement over "latest backup" — repeated sync/eject cycles can never launder or lose configs); Matryoshka 256-dim applies to GEMMA ONLY — MiniLM keeps native 384 (truncating a non-MRL model scrambled geometry, live-verified) — and Gemma's mandated task prefixes (`task: search result | query:` / `title: none | text:`) are implemented; hybrid fusion min-max normalizes the cosine channel per draft (raw (cos+1)/2 turned 70/30 into ~10/90, live-measured); the ε challenger MECHANISM is deliberately unbuilt (schema + exclusions exist; ships post-launch as opt-in); telemetry is a local consent flag only — no event builder or endpoint exists; the Lab-priors static file is not yet shipped; sqlite-vec was REMOVED as a dependency (brute-force cosine is the shipped path at ≤2K entries); HTTP transport, `Mcp-*` headers, and the `http_5xx` class defer together with URL backends; the `call` meta-tool uses `__` ids, optional `args`, and a `draft_id` field; beyond-spec hardening added: immediate write transactions + prune grace window + atomic roster.json writes (multi-client boot races), and a model-switch guard that wipes stale OATS vectors and cached need vectors when the embedding model changes.
...
| Embedding model | EmbeddingGemma-300M (ONNX confirmed in transformers.js/Node; first-run HF fetch — avoids Gemma-Terms flow-down compliance + npm bloat; check for an Apache-2.0 Gemma-4-gen successor at build time); auto-select by RAM (≥8GB → Gemma, else MiniLM-L6); lazy-load/idle-unload; vectors at 256-dim Matryoshka; `--quality` = Qwen3-Embedding-0.6B (Apache 2.0, base of ToolRet-SOTA retrievers) |
