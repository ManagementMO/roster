...
tick(9.22, 0.04, 0);
  ...
  // Keep a restrained pulse under the outcome-memory beat. The source
  // profile has a deliberate breath here, but a completely silent tail made
  // the final orbit feel disconnected in a social playback loop.
  bass(8.62, 49, 0.08, 1.46);
  [8.72, 9.04, 9.36, 9.68, 10.0].forEach((time, index) => {
    pluck(time, [392, 493.88, 523.25, 587.33, 659.25][index], 0.05, -0.45 + index * 0.22, 0.42);
  });
  for (let time = 8.76, step = 0; time < 10.08; time += 0.22, step += 1) {
    tick(time, step % 2 === 0 ? 0.024 : 0.014, step % 2 === 0 ? -0.4 : 0.4);
  }
  vacuum(10.12, 0.34);
