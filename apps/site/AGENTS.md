...
- Design tokens are in `src/styles/tokens.css`. Use native vendor colors. The owner-approved slim five-part R uses canonical SVG paths from `assets/brand/roster-r-01`; import these directly rather than tracing the obsolete bitmap. The full logo is one word: the five-part R is the initial letter followed by outlined Space Grotesk `oster`. Never pair the symbol with another full `Roster`. Primary website branding and favicons use cobalt on pearl, with the pearl inverse for dark placements. Standalone R assets are for icon-only placements.
...
- The compact, centered hero uses original-color vendor logos with CSS perspective/transform/opacity motion. The owner requested no visible pause control, so its single introduction decelerates and settles within 4.5 seconds; scrolling back or toggling reduced motion must not restart it. Motion also pauses off-screen and in hidden tabs, and reduced motion uses the static composition immediately. Keep the vendor artwork independent of Roster's identity files and logo-generation scripts. Render each hero brand exactly once; never duplicate a logo to fill another depth layer. Shared lanes contain different brands with a half-cycle offset. Run `e2e/hero.spec.ts`, `e2e/logo-uniqueness.spec.ts` and `e2e/logo-spacing.spec.ts` when changing logos or trajectories.
