...
**Two honest v1 limitations of the local quarantine** (the public League re-verifies independently, so these affect the local router's default rosters, not published scores): (1) a local quarantine currently *lifts on a 24-hour stable-re-sight dwell*, not on a completed re-Combine — the "pending re-Combine" language above describes the League gate, which is human-signed and separate; a `roster combine self` gate on local un-quarantine is planned. (2) Drift is tracked per capability *id*, so a tool that changes its entire signature but keeps its id inherits its predecessor's local **rating** until fresh outcomes move it — the Wilson/OATS outcome history is keyed by id and a drift event does *not* reset it, so stale outcomes age out only as the nightly OATS job's trailing 90-day window rolls past them. The learned **vector**, by contrast, is *not* inherited: the drift event deletes the tool's `vec` row outright (base *and* the `adj` overlay), so its base re-embeds fresh at the next warmup and `adj` is recomputed by OATS from that tool's own new outcomes — no lingering "adj ghost." Both are disclosed rather than silently carried.
