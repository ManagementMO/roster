...
const PKG_SPEC = "@npmmo/roster@0.0.1";
...
async function caseRegistryPublic() {
  await testCase("REG-public-availability", { area: "registry", title: "Approved release is available from public npm", source: "public-npm", expected: "unauthenticated HTTP and npm view return 0.0.1; unscoped `roster` never installed" }, async () => {
    ...
    const direct = await fetch(`${REGISTRY}@npmmo%2froster`, { headers: { accept: "application/json" } });
    const r = npm(["view", PKG, "version", "--registry", REGISTRY, `--@npmmo:registry=${REGISTRY}`, "--json"], { cwd: root.project, env });
    assert(direct.status === 200 && r.exitCode === 0, `public release unavailable: HTTP ${direct.status}, npm exit ${r.exitCode}`);
    assert(JSON.parse(r.stdout) === "0.0.1", `unexpected public latest version: ${r.stdout}`);
    const unscoped = npm(["view", "roster", "name", "version", "description", "--registry", REGISTRY, "--json"], { cwd: root.project, env });
    ...
    return {
      actual: `public @npmmo/roster@0.0.1 available; unscoped roster: ${unscopedInfo} (unrelated, NOT installed)`,
      notes: ["All named package installs in this run use public npm, not a staging registry"],
    };
  });
}

async function caseRegistryArtifact() {
  await testCase("REG-public-artifact-parity", { area: "registry", title: "Public npm serves the exact owner-approved artifact", expected: "public 0.0.1 and latest tag match the reviewed archive and integrity" }, async () => {
    const packument = await (await fetch(`${REGISTRY}@npmmo%2froster`)).json();
    const versions = Object.keys(packument.versions ?? {});
    const dist = packument.versions?.["0.0.1"]?.dist;
    assert(dist, `public npm does not serve 0.0.1: ${JSON.stringify(versions)}`);
    ...
    assert(packument["dist-tags"]?.latest === "0.0.1", "latest tag does not point to the approved version");
    ...
    assert(sha512b64(served) === dist.integrity && dist.integrity === TARBALL_INTEGRITY, "public artifact integrity differs from the approved candidate");
    return { actual: `versions=${JSON.stringify(versions)} dist-tags=${JSON.stringify(packument["dist-tags"])} servedSha256=${servedSha} integrity=${dist.integrity}` };
  });
}
...
async function caseLocalInstallRegistry(routes) {
  await testCase("INST-local-registry", { area: "install", route: "npm local-project (public npm)", title: "npm install @npmmo/roster@0.0.1 by name from the public npm registry", expected: "lock resolves to public npm with the owner-approved candidate integrity" }, async () => {
...
async function caseGlobalInstall(routes) {
  await testCase("INST-global-prefix", { area: "install", route: "npm global (user prefix)", title: "npm install -g @npmmo/roster@0.0.1 --prefix <user prefix>; real `roster` shim from fresh shells and multiple cwds", expected: "exit 0; roster/roster.cmd/roster.ps1 resolve via PATH in bash/sh (Linux) or cmd/powershell/pwsh (Windows) from several working directories" }, async () => {
...
async function caseNpxEphemeral(routes) {
  await testCase("INST-npx-ephemeral", { area: "install", route: "npx ephemeral (public npm)", title: "Genuine `npx -y @npmmo/roster@0.0.1 --help` with a fresh cache from fresh shells", expected: "npx fetches from the public npm registry into <cache>/_npx and prints help; also `npm exec -y`" }, async () => {
...
async function caseUpgradeNotRun() {
  await testCase("INST-version-upgrade", { area: "install", route: "npm global", title: "Genuine version upgrade from an earlier published release", expected: "n/a" }, async () => {
    notRun("NOT RUN — this is the first @npmmo/roster publication; no earlier release artifact is available for a genuine version upgrade");
  });
}
