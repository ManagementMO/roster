...
The public npm release remains **`0.0.2`**; these are candidate observations, not a `0.0.3` publication or new human-review attestation. Claude's MCP connection succeeded, but its model request was blocked by account credit; the owner directed further work toward Codex. Cursor/OpenClaw GUI journeys and ARMv7 dense support remain unverified. The upstream unusual-path limitations above are not claimed fixed. No telemetry endpoint, named League score, signing attestation, paid upgrade, or additional analytics/database service was introduced.
