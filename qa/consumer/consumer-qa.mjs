...
async function caseRegistryPublic() {
  await testCase("REG-public-availability", { area: "registry", title: `Public npm availability of ${PKG_SPEC} (reported separately)`, source: "public-npm", expected: "observed state reported; unscoped `roster` never installed" }, async () => {
    ...
    const direct = await fetch(`${publicRegistry}@npmmo%2froster/0.0.2`, { headers: { accept: "application/json" } });
    const directBody = trimTo(await direct.text(), 200);
    note(`direct GET ${publicRegistry}@npmmo%2froster/0.0.2 → HTTP ${direct.status} ${directBody}`);
    const r = npm(["view", PKG_SPEC, "version", "--registry", publicRegistry, `--@npmmo:registry=${publicRegistry}`, "--json"], { cwd: root.project, env });
    ...
    return {
      ...
      actual: `public ${PKG_SPEC}: ${publicState}; unscoped roster: ${unscopedInfo} (unrelated, NOT installed)`,
...
async function caseRegistryStaging() {
  await testCase("REG-staging-parity", { area: "registry", title: "Loopback staging registry serves the exact candidate tarball", expected: "served tarball SHA-256 == candidate; only version 0.0.2" }, async () => {
    const packument = await (await fetch(`${REGISTRY}@npmmo%2froster`)).json();
    const versions = Object.keys(packument.versions ?? {});
    const dist = packument.versions?.["0.0.2"]?.dist;
    assert(dist, `staging registry does not serve 0.0.2: ${JSON.stringify(versions)}`);
...
async function caseNpxEphemeral(routes) {
  await testCase("INST-npx-ephemeral", { area: "install", route: "npx ephemeral (staging registry)", title: "Genuine `npx -y @npmmo/roster@0.0.2 --help` with a fresh cache from fresh shells", expected: "npx fetches from the loopback staging registry into <cache>/_npx and prints help; also `npm exec -y`" }, async () => {
...
async function caseNpxSyncJourney(routes) {
  await testCase("INST-npx-sync-journey", { area: "install", route: "npx ephemeral (staging registry)", title: "npx init/sync writes the `npx -y @npmmo/roster serve` tuple; exact tuple launched from another process/cwd with baseline PATH, then after moving and after removing ONLY the _npx cache", expected: "entry is a platform-correct scoped launcher without installation/cache paths; exact command and args succeed with shell:false in all three launches; a fresh _npx entry is re-fetched" }, async () => {
...
async function caseUpgrade() {
  await testCase("INST-version-upgrade", { area: "install", route: "public 0.0.1 to candidate 0.0.2", title: "Upgrade a genuine public 0.0.1 install without changing local state or breaking its saved launcher", expected: "0.0.1 installs from public npm; candidate upgrade preserves state/backups/client config; the old launcher and scoped eject still work" }, async () => {
    ...
    return { actual: `public 0.0.1 → candidate 0.0.2; state, backups, and client config unchanged by install; old saved launcher served ${tools.length} tools and exited 0; scoped sync/eject restored exact bytes` };
  });
}
