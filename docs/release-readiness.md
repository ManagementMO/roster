...
The [public consumer campaign](https://github.com/ManagementMO/roster/actions/runs/34872239235) passed all five copied-command legs: Linux sh, macOS sh, Windows CMD, Windows PowerShell, and ARM64 PowerShell. The full consumer matrix recorded 151 PASS, 2 FAIL, 4 BLOCKED, and 8 explicitly NOT RUN observations. All five genuine public `0.0.2` upgrades passed. The two failures were the deliberate damaged-runtime repair case on Linux and Windows Node 24.20.0; the dependent dense tests were then blocked rather than falsely passed. Linux Node 22.17.0 and Windows Node 22.17.0/24.2.0 cleared their applicable cases.
...
At publication, native GUI-client/account workflows, a non-admin Windows profile, and Windows packet capture had not been exercised. The main matrix's unprivileged Linux namespace probe was unavailable; the separate packet-capture evidence below exercised the byte-identical runtime as a non-root user inside a namespace created by privileged tooling. The publication authorization did not include League signing, named scores, domains, or service deployment. The later verification and separately authorized website deployment are recorded below.

## Post-publication verification and website deployment — 2026-09-14

The independent [24-row native campaign](https://github.com/ManagementMO/roster/actions/runs/34748960570) tested the same `0.0.2` SHA-256 in every row. It recorded 563 PASS, 138 explicitly NOT RUN, 18 BLOCKED, and 1 FAIL case observations. Nineteen supported rows and two genuine Windows standard-user rows cleared their applicable gates; two older-Windows-runtime controls passed by refusing before mutation and do not count as supported-platform coverage. Missing, duplicate, unexpected, or unjustifiably skipped cases failed the gate. The blocked cases were independently reproduced upstream Node/npm path limitations, not renamed product passes.

The standard-user runs used real non-administrator accounts with medium-integrity tokens on Windows x64 and ARM64. This closes the earlier non-admin-profile gap. The remaining native failure was actual dense inference on Intel macOS: ONNX Runtime 1.24.3 has no `darwin/x64` binding, while Roster still reported the runtime installed/ON. Lexical calls and shutdown remained functional. The local 12-combination Debian/Alpine, ARM64/AMD64, Node 22/24/26 campaign verified core routing, restoration, and network-removed operation; stock Alpine's dense native load failed because the glibc loader was absent. The [retained QA code](https://github.com/ManagementMO/roster/tree/205df8ed5c070f11034aa10ca49591eafa5807bc/qa/wide) includes stock-platform reproductions and a Debian control. Local AMD64 Docker runs were emulated, not native-hardware benchmarks. An additional ARMv7 lexical run passed, but its dense installation hit a registry idle timeout; it did not establish ARMv7 dense support.

The owner separately approved the npx-first website flow and Vercel production deployment. [PR #43](https://github.com/ManagementMO/roster/pull/43) landed the one-command flow; the exact copied commands also passed [native-shell verification](https://github.com/ManagementMO/roster/actions/runs/34775227629) on Linux sh and Windows x64 CMD/PowerShell plus ARM64 PowerShell. [PR #44](https://github.com/ManagementMO/roster/pull/44) versioned the build configuration and deployed automatically from GitHub `main` commit `eb72a0ae6b217d293c97e9eccf6296d64fbac9fd`. Vercel project `roster` in `managementmos-projects` serves **https://roster-router.vercel.app**; the occupied `roster.vercel.app` was not reassigned. A branch push produced a preview and the merge produced production without a manual redeploy. Public HTTPS, canonical/social URLs, sitemap origin, and custom 404 were checked, and all 43 production-applicable browser cases passed. The local no-origin metadata case is not counted as a production pass.

The `0.0.3` portability candidate adds actual backend readiness probing and a native-first/WASM-worker fallback with private model caching. Local verification passed 568 tests, source/test typechecks, lint, clean packed installation/upgrade, and native MiniLM inference. Compiled WASM workers ran both MiniLM and EmbeddingGemma, including stock Alpine ARM64 with networking removed and a populated read-only model cache.

[PR #45's portability campaign](https://github.com/ManagementMO/roster/actions/runs/34817322876) then passed **19 configurations**: 13 native Windows/macOS/Linux combinations plus six stock-Alpine ARM64/AMD64 Node 22/24/26 combinations. All rows used the producer tarball with SHA-256 **`338e8a7e8b2d929be402e942acca26c22ce67837029e5950122970c5d820969e`**. Its five package members are byte-identical to the locally tested tarball `3ba8e57387817fcf6c0810ced0890eba69d0c9a01a0d5fa30c1b4281fd000327`; only archive encoding differs. The 25 checked consumer reports comprise 19 online runs and six fresh-process Alpine runs after network removal. Eleven reports exercised native inference and fourteen exercised WASM, including real Intel Mac inference. Each asserted actual base/need vectors and exact restoration; offline runs had to create a new query vector rather than reuse an existing database entry. Five selected rows also upgraded public `0.0.2`, preserving configuration/backups and using the previously saved launcher. These are configuration/report counts, not distinct-feature counts. [Source-commit CI](https://github.com/ManagementMO/roster/actions/runs/34817322878) and CodeQL also passed for this tested candidate.

[Windows default-configuration network tracing](https://github.com/ManagementMO/roster/actions/runs/34868853827) passed on x64 and ARM64 using the same candidate. The instrument initialized the actual CLI with `--no-dense`, verified default telemetry OFF and embeddings `auto`, confirmed no optional runtime, and did not force `ROSTER_NO_FETCH` or embeddings OFF. Kernel network ETW events were attributed using payload PIDs, not unreliable event-header PIDs. The separate HTTPS positive control produced 8 and 12 events respectively; Roster and its local stdio fixture produced zero, with no unattributed events. This is process-attributed network-event evidence for the measured fixture window, not packet-payload inspection or a claim that arbitrary configured backends never use the network. [The retained instrument](https://github.com/ManagementMO/roster/tree/dfec2af5b023e8f2c34ce520eab417ff2ec514d3/qa) keeps raw ETL data on the ephemeral runner and uploads only filtered counts and diagnostics.

Owner-authorized tests in signed-in Codex CLI `0.154.0` passed four real model-driven scenarios: transparent read, five-mode draft/call read, expected tool-error recovery, and five-mode write/read roundtrip. Success required real backend-call records, an unknown verification value in the final agent answer, and an independently checked stored note for the write/read case. The [sanitized record](verification/2026-09-14-codex-acceptance.json) preserves the rejected harness attempts as context: the normal tool host must remain enabled, and five-mode approvals were applied only to the explicitly authorized disposable fixture for that invocation. Product annotations and saved personal permissions were not weakened to obtain a pass.

These paragraphs preserve the pre-publication `0.0.3` evidence; the publication and later repair finding are recorded above. Automated observations are not an independent human-review attestation. Claude's MCP connection succeeded, but its model request was blocked by account credit; the owner directed further work toward Codex. Cursor/OpenClaw GUI journeys and ARMv7 dense support remain unverified. The upstream unusual-path limitations above are not claimed fixed. No telemetry endpoint, named League score, signing attestation, paid upgrade, or additional analytics/database service was introduced.
