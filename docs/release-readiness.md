...
A `0.0.3` portability candidate is being verified separately. It adds actual backend readiness probing and a native-first/WASM-worker fallback with private model caching; local source verification currently records 568 passing tests, typecheck/lint, clean packed installation/upgrade, and live native MiniLM inference. Compiled WASM workers ran both MiniLM and EmbeddingGemma, including a stock Alpine ARM64 run with Docker networking removed and the populated model cache mounted read-only. These are candidate observations, not an npm-publication claim or a substitute for the remaining packaged native-platform checks. The public npm release remains `0.0.2` until a newly verified artifact receives its release approval.

Native GUI/account-backed client journeys and Windows process-attributed packet capture remain separate verification gaps. No new telemetry endpoint, named League score, signing attestation, or paid infrastructure was introduced.
