...
//
// Runs OUTSIDE the repository clone against a candidate @npmmo/roster tarball and
// the public npm registry, as a brand-new unauthenticated npm/npx user would:
...
const PKG_SPEC = "@npmmo/roster@0.0.2";
...
const ENV_FACTS = {
  ...
  executionPolicy: WIN ? versionOf(PWSH_DIRS.length ? "pwsh.exe" : "powershell.exe", ["-NoProfile", "-Command", "Get-ExecutionPolicy -List | ConvertTo-Json -Compress"]) : null,
  privileges: whoami(),
  tarball: TARBALL,
  ...
  start() {
    ...
    this.exitPromise = new Promise((resolve) => {
      this.child.on("error", (err) => {
        ...
        this.exit = { code: null, signal: "SPAWN_FAILED", at: Date.now() - started };
        resolve(this.exit);
      });
...
function findRosterEntry(servers) {
  const entries = Object.entries(servers).filter(([, v]) => Array.isArray(v.args) && v.args.at(-1) === "serve" && (/roster|node|npx/i.test(String(v.command)) || v.args.includes(PKG)));
  return entries;
}
...
async function caseRegistryPublic() {
  await testCase("REG-public-availability", { area: "registry", title: `Public npm availability of ${PKG_SPEC}`, source: "public-npm", expected: "unauthenticated HTTP and npm view return the approved version; unscoped roster never installed" }, async () => {
    const root = makeRoot("reg-public");
    // The consumer npmrc maps @npmmo explicitly to public npm; a scoped registry
    ...
    const direct = await fetch(`${REGISTRY}@npmmo%2froster/0.0.2`, { headers: { accept: "application/json" } });
    ...
    assert(JSON.parse(r.stdout) === "0.0.2", `unexpected public version: ${r.stdout}`);
...
async function caseRegistryArtifact() {
  await testCase("REG-public-artifact-parity", { area: "registry", title: "Public npm serves the exact approved documentation-only patch", expected: "public 0.0.2, latest tag, SHA-256, and integrity match the reviewed archive" }, async () => {
    const packument = await (await fetch(`${REGISTRY}@npmmo%2froster`)).json();
    const dist = packument.versions?.["0.0.2"]?.dist;
    assert(dist, "public npm does not serve 0.0.2");
    ...
    assert(packument["dist-tags"]?.latest === "0.0.2", "latest tag does not point to the approved version");
    ...
    return { actual: `public 0.0.2; latest=0.0.2; servedSha256=${servedSha}; integrity=${dist.integrity}` };
  });
}
...
function assertNoWorkspaceLeak(pkgDir) {
  ...
  assert(siblings.length === 1 && siblings[0] === "roster", `unexpected @npmmo packages installed: ${siblings.join(", ")}`);
  assert(!exists(path.join(path.dirname(scopeDir), "@roster")), "unpublished workspace scope leaked into the install");
  ...
  assert(pj.name === PKG && pj.version === "0.0.2", `installed ${pj.name}@${pj.version}`);
...
function resolvedDeps(lockPath, prefixKey = "node_modules/") {
  ...
  const wanted = ["@modelcontextprotocol/sdk", "better-sqlite3", "cross-spawn", "ajv", "yaml", "smol-toml", "@npmmo/roster"];
  const out = {};
...
async function caseLocalInstallTarball(routes) {
  await testCase("INST-local-tarball", { area: "install", route: "npm local-project (tarball)", title: "npm install <candidate.tgz> in a fresh project; real shims via fresh shells", source: "candidate-tarball", expected: "exit 0; only @npmmo/roster under node_modules/@npmmo; shims run `roster --help` from fresh shells; no @huggingface" }, async () => {
    ...
    const lockSelf = readJson(path.join(root.project, "package-lock.json")).packages["node_modules/@npmmo/roster"];
    const notes = [];
    note(`resolved deps: ${JSON.stringify(deps.deps)}`, `lock entry for @npmmo/roster: ${JSON.stringify(lockSelf)}`);
...
async function caseLocalInstallRegistry(routes) {
  await testCase("INST-local-registry", { area: "install", route: "npm local-project (public npm)", title: "npm install @npmmo/roster@0.0.2 by name from the public npm registry", expected: "lock resolves to public npm with the approved artifact integrity" }, async () => {
    ...
    const lock = readJson(path.join(root.project, "package-lock.json")).packages["node_modules/@npmmo/roster"];
...
async function caseGlobalInstall(routes) {
  await testCase("INST-global-prefix", { area: "install", route: "npm global (user prefix)", title: "npm install -g @npmmo/roster@0.0.2 --prefix <user prefix>; real `roster` shim from fresh shells and multiple cwds", expected: "exit 0; roster/roster.cmd/roster.ps1 resolve via PATH in bash/sh (Linux) or cmd/powershell/pwsh (Windows) from several working directories" }, async () => {
...
async function caseNpxEphemeral(routes) {
  await testCase("INST-npx-ephemeral", { area: "install", route: "npx ephemeral (public npm)", title: "Genuine `npx -y @npmmo/roster@0.0.2 --help` with a fresh cache from fresh shells", expected: "npx fetches from the public npm registry into <cache>/_npx and prints help; also `npm exec -y`" }, async () => {
    ...
    const installed = npxDirs.map((d) => path.join(root.npmCache, "_npx", d, "node_modules", "@npmmo", "roster", "package.json")).filter(exists);
    assert(installed.length > 0, "npx cache does not contain @npmmo/roster");
    const lock = readJson(path.join(root.npmCache, "_npx", npxDirs[0], "package-lock.json"));
    const self = Object.entries(lock.packages).find(([k]) => k.endsWith("node_modules/@npmmo/roster"))?.[1];
...
async function caseNpxSyncJourney(routes) {
  await testCase("INST-npx-sync-journey", { area: "install", route: "npx ephemeral (public npm)", title: "npx init/sync writes the `npx -y @npmmo/roster serve` tuple; exact tuple launched from another process/cwd with baseline PATH, then after moving and after removing ONLY the _npx cache", expected: "entry is a platform-correct scoped launcher without installation/cache paths; exact command and args succeed with shell:false in all three launches; a fresh _npx entry is re-fetched" }, async () => {
    ...
    const expectedEntry = WIN
      ? { command: path.win32.join(process.env.SystemRoot ?? "C:\\Windows", "System32", "cmd.exe"), args: ["/d", "/s", "/c", "npx", "-y", PKG, "serve"] }
      : { command: "npx", args: ["-y", PKG, "serve"] };
    assert(entry.command === expectedEntry.command && JSON.stringify(entry.args) === JSON.stringify(expectedEntry.args), `unexpected tuple: ${raw}`);
    ...
    const launch = async (label) => {
      ...
      const client = new McpClient(entry.command, entry.args, { ...spawnOpts, shell: false }).start();
      let ok = false;
      let detail;
      try {
      ...
      ok = ok && exit.code === 0 && !exit.timedOut;
    ...
    results.first = await launch("exact generated tuple with shell:false");
    assert(results.first.ok, `exact tuple failed: ${results.first.detail}`);
    fs.renameSync(npxCache, `${npxCache}-moved-${rand()}`);
    results.moved = await launch("launch after MOVING _npx");
    ...
    results.removed = await launch("launch after REMOVING _npx");
    ...
    note(`fresh ${SHELLS[0]} \`npx -y @npmmo/roster --help\`: exit ${shellTuple.exitCode}`);
    assert(shellTuple.exitCode === 0 && /roster init/.test(shellTuple.stdout), "fresh-shell help failed");
    ...
    return { actual: `tuple ${raw}; all three launches use shell:false; moved=${results.moved.ok} removed=${results.removed.ok} (refetched ${results.moved.fresh.length}/${results.removed.fresh.length} new _npx entries); eject byte-identical`, notes };
  });
}

async function caseUpgrade() {
  await testCase("INST-version-upgrade", { area: "install", route: "public 0.0.1 to public 0.0.2", title: "Upgrade a genuine public 0.0.1 install without changing local state or breaking its saved launcher", expected: "0.0.1 installs from public npm; candidate upgrade preserves state/backups/client config; the old launcher and scoped eject still work" }, async () => {
    const root = makeRoot("version-upgrade");
    const env = envFor(root);
    const prior = npm(["install", "-g", "@npmmo/roster@0.0.1", "--prefix", root.prefix, "--registry", "https://registry.npmjs.org/", "--@npmmo:registry=https://registry.npmjs.org/"], { cwd: root.elsewhere, env });
    assert(prior.exitCode === 0, `public 0.0.1 install failed: ${prior.exitCode}`);
    const manifestPath = path.join(globalPkgDir(root.prefix), "package.json");
    assert(readJson(manifestPath).version === "0.0.1", "upgrade baseline is not the genuine 0.0.1 package");
    const fx = seedFixtures(root);
    const roster = rosterRunner(root);
    assert(roster(["init", "--no-dense"]).exitCode === 0, "prior-version init failed");
    assert(roster(["sync", "--client", "cursor"]).exitCode === 0, "prior-version sync failed");
    const entry = findRosterEntry(mcpServersOf(fx.files.cursor))[0]?.[1];
    assert(entry, "prior version did not write a saved launcher");
    const snapshot = () => listFiles(root.rosterHome).sort().map((file) => [path.relative(root.rosterHome, file), sha256(fs.readFileSync(file))]);
    ...
    assert(readJson(manifestPath).version === "0.0.2", "upgrade did not install the candidate version");
    ...
    try {
      ...
      exit = await client.eof(30_000);
    }
    assert(tools.some((tool) => /create_entities/.test(tool.name)), "saved prior-version launcher lost the real backend");
    assert(exit.code === 0 && !exit.timedOut, "upgraded launcher did not shut down cleanly");
    ...
    return { actual: `public 0.0.1 → public 0.0.2; state, backups, and client config unchanged by install; old saved launcher served ${tools.length} tools and exited 0; scoped sync/eject restored exact bytes` };
  });
}
...
async function networkEvidence(routes) {
  ...
  await testCase("NET-telemetry-off-strace", { area: "network", route: "npm global", title: "Process-attributed network evidence: installed product commands + a transparent serve session make no non-loopback connections (strace -f -e connect,sendto,sendmsg)", expected: "strace shows no connect()/sendto()/sendmsg() to non-loopback AF_INET/AF_INET6 for init --no-dense/receipt/telemetry status/sync/eject/dense status and a serve session" }, async () => {
    if (WIN) notRun("NOT RUN on Windows — this harness does not collect process-attributed packet/syscall evidence on Windows");
  ...
  await testCase("NET-offline-namespace", { area: "network", route: "npm global", title: "Installed product commands + serve session succeed with networking removed (unshare -r -n)", expected: "receipt/telemetry status/sync/eject and a serve session succeed inside a network-less user namespace; NOT RUN if unshare -n is unavailable on this host" }, async () => {
    if (WIN) notRun("NOT RUN on Windows — network isolation is not implemented by this harness on Windows");
...
if (flag("diagnostics")) await diagnostics();
