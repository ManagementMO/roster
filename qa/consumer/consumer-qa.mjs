...
async function caseRegistryPublic() {
  await testCase("REG-public-availability", { area: "registry", title: "Public npm availability of @npmmo/roster (must be reported separately)", source: "public-npm", expected: "observed state reported; unscoped `roster` never installed" }, async () => {
    ...
    const direct = await fetch(`${publicRegistry}@npmmo%2froster`, { headers: { accept: "application/json" } });
    const directBody = trimTo(await direct.text(), 200);
    note(`direct GET ${publicRegistry}@npmmo%2froster → HTTP ${direct.status} ${directBody}`);
    const r = npm(["view", PKG, "version", "--registry", publicRegistry, `--@npmmo:registry=${publicRegistry}`, "--json"], { cwd: root.project, env });
    ...
    return {
      ...
      actual: `public @npmmo/roster: ${publicState}; unscoped roster: ${unscopedInfo} (unrelated, NOT installed)`,
...
async function caseLocalInstallRegistry(routes) {
  await testCase("INST-local-registry", { area: "install", route: "npm local-project (staging registry)", title: "npm install @npmmo/roster@0.0.1 by name from the loopback staging registry", expected: "lock resolves to 127.0.0.1 registry with candidate integrity" }, async () => {
...
async function caseNpxEphemeral(routes) {
  await testCase("INST-npx-ephemeral", { area: "install", route: "npx ephemeral (staging registry)", title: "Genuine `npx -y @npmmo/roster@0.0.1 --help` with a fresh cache from fresh shells", expected: "npx fetches from the loopback staging registry into <cache>/_npx and prints help; also `npm exec -y`" }, async () => {
...
async function caseUpgradeNotRun() {
  await testCase("INST-version-upgrade", { area: "install", route: "npm global", title: "Genuine version upgrade from an earlier published release", expected: "n/a" }, async () => {
    notRun("NOT RUN — no earlier @npmmo/roster release artifact exists (public npm E404; staging holds only 0.0.1)");
  });
}
