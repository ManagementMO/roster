...
| **M5 — synced entry pointed at a missing binary** (`cli/sync.ts`) | `command: "roster"` fails to spawn for `npx -y @roster/cli init` users with no global install. | npx-aware entry when no global `roster` is on PATH; `isAlreadySynced` recognizes both forms. | cli.test |
...
## Round 4c — independent clean-room re-review (Fable 5, 2026-07-07)

Then a *clean-room* re-review: a fresh reviewer with no memory of the fix rationale, told only "find what's wrong." Grade **B−**, 7 findings (DEF-1…7). All confirmed and fixed — none CRITICAL, but three were correctness bugs the same-author passes had walked straight past.

| Finding | Confirmed how | Fix |
|---|---|---|
| **DEF-1 — D4×model-switch, the sibling of 4b's D4×drift I'd missed.** `ensureEmbeddingModel` nulled `adj` on a model switch but LEFT the base `vec` row; the D4 warm-boot backfill skips ids that still have a row → a MiniLM↔Gemma switch pinned the old-dimension base vector forever, silently poisoning retrieval. | `:memory:` probe: switch dims → base row survives at the wrong dims, backfill skips it. | `DELETE FROM vec` on model switch (same invalidation as drift) so warm-boot re-embeds every base at the new dims; `adj` regenerates from outcomes. `serve.ts` comment corrected to name BOTH invalidators. Regression test. |
| **DEF-5 — `hasGlobalRoster` trusted `existsSync`, a smaller replay of 4b's squatter hazard.** A third-party `roster` on PATH — or merely a *directory* named `roster` — would be written into every client as a spawn target. | PATH semantics; the npm `roster` is confirmed third-party (4b). | Require an executable regular FILE that `realpath`s INTO this checkout (no global `roster` today is ours; relaxes at publish). `ROSTER_ASSUME_GLOBAL` override. |
| **DEF-4 — `isAlreadySynced` false-positived on stale/moved entries.** The loose check (`command==="roster"` OR args include `serve`) treated a moved/removed install — or a user's own server keyed "roster" — as healthy, so sync never refreshed a broken entry. | Traced the branch against a gained-global + moved-install matrix. | Recognize our EXACT current entry, or our own `bin.js` path (authoritative even after the machine gains a global — a working execPath entry isn't churned, M5), or a *different* `bin.js` that still exists on disk; a vanished script falls through to a refresh. Mutation-checked live: the M5 test failed before the `ourBin` clause, passed after. |
| **DEF-6 — classifier kind heuristics misroute ~24% of a realistic corpus, all auth-direction; docstring stale.** Bare `token`/`auth` and quoted echoed paths caused 11/46 real-wire misroutes (`notes-classifier-realworld.md`); the docstring claimed "auth before schema" while the code does the reverse. | Re-scored the lab's 46-text corpus against the built classifier (11 wrong). | Adopted the lab's *validated* rules — strip quoted literals, CONTEXTUAL token (not bare), underscore-tolerant auth, access-denied, 5xx — while KEEPING the §8-fairness ordering (internal-before-schema, schema-before-auth) and the richer M4 quota/schema regexes rather than the proposal's flatter order. 27/27 (11 documented + 16 held-out). Docstring rewritten to the true order + rationale. |
| **DEF-2 — "byte-for-byte" over-promise in README/`bin`/`init`/`eject`.** Eject restores byte-for-byte for DEDICATED configs but KEY-LEVEL for live state files (`~/.claude.json`); the docs promised byte-for-byte universally. | Read the eject branches vs the copy. | Tempered every promise to distinguish the two modes; `stateFile?: boolean` → `stateFileBasename?: string` so a client's own dedicated file (Claude Code's cwd `.mcp.json`) still gets the byte path. |
| **DEF-3 — behaviors without a test lock.** D8 array-config refusal, D2 BOM tolerance, and the DEF-6 anti-false-positive rules had no regression test. | Grep of the test tree. | Added locks: array-config → throws + config left byte-identical; BOM'd config → still imports (`result.imported===1`) + clean roster-only rewrite; a classifier hardening block incl. the §8 internal-vs-schema ambiguity. Mutation-checked (revert 5xx → 502/503 go red). |
| **DEF-7 — forward-only migration undisclosed.** The coach DB migrates additively (`CREATE TABLE IF NOT EXISTS`); `schema_version` is written once and never re-read — no down-migration, no newer-binary guard. Fine at v1, but silent. | Read `db.ts`. | Disclosure comment at `SCHEMA_VERSION`/`migrate()` naming exactly what a breaking bump must add; tracked as a pre-scale P-item. |

Process note: while hardening DEF-5, an Edit mis-matched a backtick region and truncated `sync.ts` (lost `hasGlobalRoster`/`rosterEntry`). Recovered by full reconstruction, then gated on `tsc -b` + the entire 161-test suite BEFORE proceeding — a corrupted file must never be built or committed on trust.

Lesson: a *clean-room* reviewer (no access to the fix rationale) catches a different class than same-author self-review — DEF-1 and DEF-5 were each "the sibling case of a bug I'd already fixed," invisible precisely because I'd convinced myself that area was done.
