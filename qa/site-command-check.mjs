...
const prefix = `${win ? "npx.cmd" : "npx"} --yes @npmmo/roster@0.0.2`;
...
const report = { platform: process.platform, arch: process.arch, node: process.version, shell, package: "@npmmo/roster@0.0.2", commands: [], startedAt: new Date().toISOString(), status: "FAIL" };
const run = (name) => {
...
try {
  ...
  const expected = "sha512-+Cw+KH1vdQfznx/0iGbZl62WkImkyLbz8F8sCU0Hj0Xd22bcfhPmHOUD04WABb2fzWpt9e89Lbc5fG8CC1YW1g==";
  assert(packages.some((entry) => entry.version === "0.0.2" && entry.integrity === expected));
