...
const ids = [
  "SETUP-pinned-backends", "REG-public-availability", "REG-public-artifact-parity",
...
const fixture = () => ({
  ...
  results: ids.map((id) => ({ id, status: "PASS", artifact: { package: "@npmmo/roster@0.0.2", tarballSha256: sha } })),
});
function run(report) {
...
test("accepts complete evidence and explicitly allowed unverified checks", () => {
  ...
  assert.match(result.stdout, /explicitly NOT RUN: NET-offline-namespace/);
});
test("requires the real previous-version upgrade", () => {
  const report = fixture();
  report.results.find((row) => row.id === "INST-version-upgrade").status = "NOT RUN";
