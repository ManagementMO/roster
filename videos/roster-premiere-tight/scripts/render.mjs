...
writeFileSync("verification/render-plan.json", JSON.stringify({
  ...
  soundtrack, duration:15, fps:120,
}, null, 2)+"\n");
...
try {
  // Keep picture rendering and soundtrack mastering separate and reproducible.
  run("npx", ["--yes","hyperframes@0.8.31","render","--composition",temporary,"--fps","120","--quality","high","--workers","2","--output","renders/roster-tight-picture-120fps.mp4"]);
  run("ffmpeg", ["-hide_banner","-y","-i","renders/roster-tight-picture-120fps.mp4","-i",soundtrack,
    "-map","0:v:0","-map","1:a:0","-c:v","copy","-c:a","aac","-b:a","256k","-t","15","-movflags","+faststart","renders/roster-tight-120fps.mp4"]);
  console.log("Rendered 120fps picture and muxed the mastered soundtrack: renders/roster-tight-120fps.mp4");
} finally {
