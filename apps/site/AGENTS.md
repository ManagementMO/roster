...
- The compact, centered hero uses original-color vendor logos with CSS perspective/transform/opacity motion. The owner requested no visible pause control. Motion stops automatically off-screen, in hidden tabs, and for reduced motion. Keep the vendor artwork independent of Roster's identity files and logo-generation scripts. Render each hero brand exactly once; never duplicate a logo to fill another depth layer. Shared lanes contain different brands with a half-cycle offset. Run `e2e/logo-uniqueness.spec.ts` and `e2e/logo-spacing.spec.ts` when changing logos or trajectories.
