...
The owner also approved work on a native-first/WASM fallback and truthful dense-runtime readiness. The resulting `0.0.3` candidate passed automated artifact verification on 19 platform configurations, plus scoped Windows network tracing and owner-authorized signed-in Codex acceptance tests. Its release approval remains separate: the prior reviewed `0.0.1`/documentation-only `0.0.2` authorization does not certify new runtime code. Claude's model-driven test was blocked by account credit, and the owner explicitly directed continued end-to-end testing toward Codex rather than changing accounts or purchasing credits.
