---
version: alpha
name: Roster Premiere — Coral adaptation
description: >
  ...
  ink black / warm cream), Space Grotesk uppercase tracked + Space Grotesk body, the 45° diagonal hatch,
...
# PRODUCTION OVERRIDES: Correct the automatic font-role mapping for this film.
# Space Grotesk variable 700 display, Manrope 400–600 support, JetBrains Mono 400 API text.
# Hero type 110–230px; hook numeral may reach 550px. Display tracking -0.045em, not wide.
# Minimum meaningful type 30px, mono notes 26px. Ignore the legacy 11–14px chrome ramps below.
# No wallpaper numerals, hatch textures, generic glyph logos or ornamental labels in this film.
# The supplied Roster logo replaces every preset brand glyph. No external font URLs.
# Product chips can use 18–28px corners and subtle material borders; macro color regions stay flat.
# Every root uses 1920x1080, background on full-duration child clip, local time starting at 0.
# Use one paused GSAP timeline per subcomposition. Root ID must be e.g. 01-overload,
# identical to data-composition-id and window.__timelines key. Prefix child IDs with s01- etc.
# No CSS transform on an element whose transform is animated by GSAP. Explicit px dimensions.
# Rely on master wrapper seams; do not animate the subcomposition root's scale/x/y/opacity.
# All content must be visible inside x=96..1824, y=80..1000; decorative tiles may crop on purpose.
...
typography:
  ...
  body:          { fontFamily: "Space Grotesk", cqw: 1.0,  weight: 400, lineHeight: 1.7 }
  body-light:    { fontFamily: "Space Grotesk", cqw: 1.5,  weight: 300, lineHeight: 1.5, note: "pull-quote voice" }
  section-label: { fontFamily: "Space Grotesk", px: 12, weight: 700, tracking: "4px", upper: true }
  item-label:    { fontFamily: "Space Grotesk", px: 11, weight: 700, tracking: "3px", upper: true }
  quote-attribution: { fontFamily: "Space Grotesk", px: 14, weight: 600, tracking: "3px", upper: true }
  quote-role:    { fontFamily: "Space Grotesk", px: 12, weight: 400, tracking: "1px" }
  # — display / hero ramp (Space Grotesk, uppercase, tracked) —
  card-title:    { fontFamily: "Space Grotesk", cqw: 1.9, weight: 400, lineHeight: 1.1, tracking: "1px", upper: true }
  sidebar-value: { fontFamily: "Space Grotesk", cqw: 2.3, weight: 400, lineHeight: 1.0, upper: true }
  bar-title:     { fontFamily: "Space Grotesk", cqw: 2.3, weight: 400, lineHeight: 1.0, tracking: "2px", upper: true }
  card-stat:     { fontFamily: "Space Grotesk", cqw: 2.5, weight: 400, lineHeight: 1.0, upper: true }
  column-title:  { fontFamily: "Space Grotesk", cqw: 3.7, weight: 400, lineHeight: 1.0, tracking: "2px", upper: true }
  section-headline:{ fontFamily: "Space Grotesk", cqw: 4.2, weight: 400, lineHeight: 1.0, tracking: "2px", upper: true }
  stat-numeral:  { fontFamily: "Space Grotesk", cqw: 5.0, weight: 400, lineHeight: 1.0, upper: true }
  hero-title:    { fontFamily: "Space Grotesk", cqw: 6.5, weight: 400, lineHeight: 0.9, tracking: "4px", upper: true }
  jumbo-feature: { fontFamily: "Space Grotesk", cqw: 9.0, weight: 400, lineHeight: 1.0, tracking: "12px", upper: true }
  # — decorative —
  background-numeral: { fontFamily: "Space Grotesk", cqw: 10.0, weight: 400, color: "rgba(0,0,0,0.12)", note: "wallpaper numeral inside a coral region" }
  giant-mark:    { fontFamily: "Space Grotesk", cqw: 14.0, weight: 400, color: "rgba(0,0,0,0.35)", note: "decorative quote mark inside a coral region" }
...
components:
  ...
  info-bar:
    ...
    rounded: "0"
    shadow: "none"
    description: "Footer band beneath a feature region — Bebas title left, Space Grotesk meta right."
---
...
- **Fonts** — already set to **Space Grotesk** (display) / **Manrope** (body); ignore any preset font name lingering in prose.
...
The voice is a two-face hierarchy: **Space Grotesk** — tall condensed caps, always uppercase, always
tracked (1–12px) — carries every headline, stat, title, and meta figure; **Space Grotesk** carries every
body line, label, and attribution across weights 300–700. Bebas declares; Space Grotesk explains. The
...
- **Bebas uppercase + tracking** on every display element; **Space Grotesk** on every body/label.
...
Two ramps. The **reading ramp** (Space Grotesk body 1.0cqw, body-light 1.5cqw, labels in px) carries copy
...
- **Every Bebas element is uppercase with ≥1px tracking** (2px standard, 4px hero, 12px jumbo). **Every Space Grotesk label is uppercase, 1–4px tracked.** No italic, no underline, no sentence-case Bebas.
...
ink `section-label` eyebrow; an optional Space Grotesk-300 support line ≤44cqw. **Accent** the coral ground
...
**Focal** a 2–3 line pull quote in `{colors.cream}` Space Grotesk **weight 300** on the ink panel.
...
mark. **Fixed** Space Grotesk-300 quote, ink panel, ink-on-coral mark. **Free** quote, attribution, mark
...
Bebas title, Space Grotesk body, coral stat). **Accent** the three coral top borders + icon squares.
...
- Set every Bebas element **uppercase + tracked** (2px standard, 4px hero, 12px jumbo); every Space Grotesk label uppercase, 1–4px.
...
- Don't render Bebas in sentence case or untracked; don't pair it with a non-Space Grotesk body sans.
...
- **Space Grotesk + Space Grotesk via Google Fonts.** CJK pairing (ZCOOL XiaoWei / Yozai) carries over from the source's CJK section; Bebas runs ~20% wider in CJK.
