...
export const drawRibbon = (ctx, start, control1, control2, end, color, width, progress = 1, alpha = 1, style = "route") => {
  ...
  const drawPath = () => {
    ...
    for (let index = 0; index <= steps; index += 1) {
      const point = bezierPoint(start, control1, control2, end, (index / steps) * amount);
  ...
  if (style === "route") {
    ...
    ctx.shadowBlur = 0;
    ctx.shadowOffsetY = 0;
    ctx.strokeStyle = "rgba(7,10,11,0.82)";
    ctx.lineWidth = width + 7;
    drawPath();
    ctx.strokeStyle = rgba(accent, 0.86);
    ctx.lineWidth = width;
    drawPath();
    ctx.strokeStyle = rgba(COLOR.white, 0.22);
    ctx.lineWidth = 1.15;
    drawPath();
  } else {
  ...
  if (style === "route" && amount > 0.03) {
    const tip = bezierPoint(start, control1, control2, end, amount);
    ctx.fillStyle = rgba(accent, 0.9);
    ctx.beginPath();
    ctx.arc(tip[0], tip[1], Math.max(3.2, width * 0.72), 0, Math.PI * 2);
    ctx.fill();
  }
  ctx.restore();
};
...
/**
 * Quiet terminal-to-tool connector. This is intentionally flatter than the
 * general ribbon treatment: one saturated ink core, a recessed graphite edge,
 * and a tiny solid signal bead. There is no bloom, inner white tube, or arrow;
 * the line should read as a calm physical path while the card does the acting.
 */
export const drawTerminalPath = (ctx, start, control1, control2, end, progress = 1, color, width = 5, alpha = 1) => {
  const amount = clamp(progress);
  if (amount <= 0) return;
  const steps = 76;
  const drawRange = () => {
    const count = Math.max(4, Math.ceil(steps * amount));
    ctx.beginPath();
    for (let index = 0; index <= count; index += 1) {
      const point = bezierPoint(start, control1, control2, end, (index / count) * amount);
      if (index === 0) ctx.moveTo(point[0], point[1]);
      else ctx.lineTo(point[0], point[1]);
    }
    ctx.stroke();
  ...
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  // The dark keyline gives the path separation on the terminal without
  // creating a glowing tube or a second bright stroke.
  ctx.shadowColor = "transparent";
  ctx.strokeStyle = "rgba(7,10,11,0.82)";
  ctx.lineWidth = width + 7;
  drawRange();
...
export const drawToolObject = (ctx, tool, x, y, width, height, scale, rotation, progress, detail = 1, dark = false, hero = false, tintStrength = null) => {
  ...
  const explicitTint = tintStrength !== null;
  const surfaceTint = tintStrength ?? (dark ? 0.22 : 0.12);
  // A zero tint is an explicit request for a neutral surface. Keep the
  // material gradient, but do not let the tool's brand colour bleed across the
  // whole card; the mark itself remains the colour cue.
  shell.addColorStop(1, explicitTint && surfaceTint === 0 ? (dark ? "#171C1A" : "#F9FBFD") : rgba(tool.color, surfaceTint));
  fillRoundRect(ctx, -width / 2, -height / 2, width, height, hero ? 34 : 26, shell);
  ctx.shadowColor = "transparent";
  const edge = dark
    ? rgba(tool.color, explicitTint ? Math.min(0.3, 0.12 + surfaceTint * 2.4) : 0.72)
    : "rgba(255,255,255,0.94)";
  strokeRoundRect(ctx, -width / 2 + 1, -height / 2 + 1, width - 2, height - 2, hero ? 34 : 26, edge, 2);
  ctx.fillStyle = rgba(tool.color, explicitTint ? Math.min(0.04, surfaceTint * 0.4) : dark ? 0.12 : 0.08);
