...
- ~~Dense retrieval end-to-end~~ **NOW LIVE-VERIFIED** (`docs/verification/dense-live.mjs` + transcript): real model download + inference through the real provider/store — hybrid fusion demotes token-noise matches, and **OATS moved a tool's cosine from ~0 to 0.491 after just 4 real success outcomes** (the learning loop demonstrably works). Two honest notes from the live run: MiniLM's *absolute* similarities on tool blurbs are near-zero (relative order is what routing uses, and it's correct — Gemma, the ≥8GB default, has stronger geometry); and Matryoshka truncation is now correctly Gemma-only (slicing MiniLM scrambled it — caught live, fixed). The Gemma-specific 200MB fetch itself hasn't been downloaded once (identical code path, larger artifact).
- **CI is green across ubuntu/macos/windows** (the failure you pasted was a platform-path bug in test *fixtures*, not product code — fixtures now derive paths from the client registry, so tests exercise real per-platform logic everywhere).
- **Ratings surfacing:** the nightly job runs and populates the ratings table, but nothing user-facing reads it yet (dashboard and League site are M2).
