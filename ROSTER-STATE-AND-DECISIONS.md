...
> **Release update (2026-09-14):** `@npmmo/roster@0.0.3` is public under `npmmo`, with executable `roster`. The owner reviewed PR #45 and approved the exact archive; merge CI/CodeQL passed and the public download matched its approved hash. The earlier `0.0.2` documentation-only release remains immutable. Older `@roster/cli` references below are decision history, not the package to install. Current evidence and the post-publication repair finding: `docs/release-readiness.md` and `docs/PROVENANCE.md`.

> **Deployment and verification follow-up (2026-09-14):** the owner approved the npx-first frontend and production deployment to Vercel project `roster`. The preferred public website is `https://roster-router.vercel.app`, attached to the project and updated by the native GitHub integration from `main`; a real branch preview and automatic post-merge production build were verified. Genuine Windows non-admin x64/ARM64 checks are now complete. Optional native dense-runtime failures on Alpine/musl and Intel macOS led to the reviewed `0.0.3` WASM-fallback/readiness release. Its 19-configuration candidate matrix, Windows network traces, and four real Codex CLI flows passed. Broader public verification then found a repair-only stale package-resolution error; the owner authorized a separately verified `0.0.4` correction, which still needs its own artifact approval. League signing, named scores, telemetry uploads, and new paid services remain outside this work. Details: `docs/release-readiness.md`.
