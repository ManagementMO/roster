...
> **Release update (2026-09-14):** `@npmmo/roster@0.0.4` is public under `npmmo`, with executable `roster`. The owner separately reviewed PR #46 and approved the exact repair-fix archive; merge CI/CodeQL passed and the public download matched its approved hash. The earlier releases and the `0.0.3` repair-failure evidence remain immutable. Older `@roster/cli` references below are decision history, not the package to install. Current evidence: `docs/release-readiness.md` and `docs/PROVENANCE.md`.

> **Deployment and verification follow-up (2026-09-14):** the owner approved the npx-first frontend and production deployment to Vercel project `roster`. The preferred public website is `https://roster-router.vercel.app`, attached to the project and updated by the native GitHub integration from `main`; a real branch preview and automatic post-merge production build were verified. Genuine Windows non-admin x64/ARM64 checks are now complete. Optional native dense-runtime failures on Alpine/musl and Intel macOS led to the reviewed `0.0.3` WASM-fallback/readiness release. Its 19-configuration candidate matrix, Windows network traces, and four real Codex CLI flows passed. Broader public verification found a repair-only stale package-resolution error. The owner then separately reviewed and approved `0.0.4`, now published after all 19 pre-release repair configurations, Windows network traces, and repeated Codex checks passed. The broader public install, upgrade, and repair matrix also passed. League signing, named scores, telemetry uploads, and new paid services remain outside this work. Details: `docs/release-readiness.md`.
